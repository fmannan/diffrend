...
def copy_scripts_to_folder(expr_dir):
    ...
    shutil.copy("parameters.py", expr_dir)
...
class GAN(object):
    ...
    def create_networks(self, ):
        """Create networks."""
        if self.opt.joint_training:
            print("joing training**")
    ...
    def train(self, ):
        ...
        with open(file_name, 'wt') as l2_file:
            for iteration in range(self.opt.n_iter):
                    ...
                    if self.opt.gp != 'None':
                        if self.opt.RGBD:
                            gradient_penalty = calc_gradient_penalty(
                                self.netD, input_D.data, fake_D.data,
                                self.opt.gp_lambda)
                ...
                fake_rendered,fd,r = self.render_batch(fake)
                if self.opt.RGBD:
                    outG_fake = self.netD(torch.cat([fake_rendered,fd],1))
                if self.opt.joint_training:
                ...
                if iteration % 5 == 0:
                    ...
                    if self.opt.joint_training:
                        ...
                        print('\n[%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_D_real: %.4f'
                              ' Loss_D_fake: %.4f Wassertein D: %.4f L2_loss: %.4f' % (
                                  iteration, self.opt.n_iter, errD.data[0],
                                  errG.data[0], errD_real.data[0], errD_fake.data[0],
                                  Wassertein_D, l2_loss.data[0]))
                    l2_file.write('%s\n' % (str(l2_loss.data[0])))
                    l2_file.flush()
