...
from diffrend.torch.GAN.two_networks_fragposition import create_networks
...
def copy_scripts_to_folder(expr_dir):
    shutil.copy("two_networks_fragposition.py", expr_dir)
...
class GAN(object):
    ...
    def create_networks(self, ):
        """Create networks."""
        self.netG, self.netD, self.netD2 = create_networks(self.opt, verbose=True)
    ...
    def create_tensors(self, ):
        """Create the tensors."""
        self.input3d = torch.FloatTensor(
            self.opt.batchSize, 6,
            self.opt.render_img_size, self.opt.render_img_size)
        self.input23d = torch.FloatTensor(
            self.opt.batchSize, 6,
            self.opt.render_img_size, self.opt.render_img_size)
        ...
        if not self.opt.no_cuda:
            self.input3d = self.input3d.cuda()
            self.input23d = self.input23d.cuda()
    ...
    def get_real_samples(self):
        ...
        for idx in range(self.opt.batchSize):
            ...
            data3d.append(torch.cat([res['pos'].permute(2, 0, 1),res['normal'].permute(2, 0, 1)],0))
            data.append(im)
            data_depth.append(im_d)
            data_cond.append(large_scene['camera']['eye'])
        # Stack real samples
        real_samples3d = torch.stack(data3d)
        ...
        if not self.opt.no_cuda:
            real_samples3d = real_samples3d.cuda()
        ...
        for idx in range(self.opt.batchSize):
            ...
            # Add depth image to the output structure
            # tmp3d=torch.cat([res['pos'],res['normal']],0)
            # import ipdb; ipdb.set_trace()
            data23d.append(torch.cat([res['pos'].permute(2, 0, 1),res['normal'].permute(2, 0, 1)],0))
            data2.append(im)
            data_depth2.append(im_d)
            data_cond2.append(large_scene['camera']['eye'])
        # Stack real samples
        real_samples23d = torch.stack(data23d)
        real_samples2 = torch.stack(data2)
        real_samples_depth2 = torch.stack(data_depth2)
        real_samples_cond2 = torch.stack(data_cond2)
        self.batch_size = real_samples.size(0)
        if not self.opt.no_cuda:
            real_samples23d = real_samples23d.cuda()
        ...
        self.input3d.resize_as_(real_samples3d.data).copy_(real_samples3d.data)
        self.input23d.resize_as_(real_samples23d.data).copy_(real_samples23d.data)
        ...
        self.inputv3d = Variable(self.input3d)
        self.inputv23d = Variable(self.input23d)
    ...
    def render_batch(self, batch, batch_cond):
        ...
        rendered_data3d = torch.stack(rendered_data3d)
        rendered_data_depth = torch.stack(rendered_data_depth)
        return rendered_data, rendered_data_depth, rendered_data3d,  loss/self.opt.batchSize
    def train(self, ):
        """Train networtk."""
        # Start training
        ...
        file_name2 = os.path.join(self.opt.out_dir, 'L1.txt')
        file_name3 = os.path.join(self.opt.out_dir, 'L2_depth.txt')
        with open(file_name, 'wt') as l2_file, open(file_name2, 'wt') as l1_file, open(file_name3, 'wt') as l3_file:
            for iteration in range(self.opt.n_iter):
                ...
                for j in range(self.opt.critic_iters):
                    ...
                    real_output = self.netD(self.inputv3d, self.inputv_cond)
                    real_output_depth = self.netD2(self.inputv23d, self.inputv_cond2)
                    ...
                    fake2 = self.netG(self.noisev2, self.inputv_cond2)
                    ...
                    outD_fake = self.netD(r.detach(), self.inputv_cond.detach())
                    outD_fake_depth = self.netD2(r2.detach(), self.inputv_cond2.detach())
                    ...
                    if self.opt.gp != 'None':
                        gradient_penalty = calc_gradient_penalty(
                            self.netD, self.inputv3d.data, r.data,self.inputv_cond.data,
                            self.opt.gp_lambda)
                        gradient_penalty.backward()
                        errD += gradient_penalty

                        gradient_penalty_depth = calc_gradient_penalty(
                            self.netD2, self.inputv23d.data, r2.data, self.inputv_cond2.data,
                            self.opt.gp_lambda)
                ...
                fake_rendered,fd,r, loss = self.render_batch(fake, self.inputv_cond)
                fake_rendered2,fd2,r2, loss2 = self.render_batch(fake2, self.inputv_cond2)
                outG_fake = self.netD(r, self.inputv_cond)
                outG_fake_depth = self.netD2(r2, self.inputv_cond2)
                ...
                if iteration % 5 == 0:
                    l2_loss_depth=mse_criterion(fd, self.inputv_depth)
                    l2_loss=mse_criterion(r, self.inputv3d)
                    l1_loss=l1_criterion(r, self.inputv3d)
                    l1_loss2=l1_criterion(r[:, :3], self.inputv3d[:, :3])+l1_criterion(r[:, 3:], self.inputv3d[:, 3:])
                    l1_loss3=l1_criterion(r2, self.inputv23d)
                    ...
                    print('\n[%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_D_real: %.4f'
                          ' Loss_D_fake: %.4f Loss_D_real_depth: %.4f Loss_D_fake_depth: %.4f Wassertein D: %.4f Wassertein_depth D_depth: %.4f loss: %.4f' % (
                    ...
                    print('\n[%d/%d] Loss_l2: %.4f Loss_l1: %.4f Loss2_l1: %.4f'
                          ' Loss3_l1: %.4f  ' % (
                              iteration, self.opt.n_iter, l2_loss.data[0],
                              l1_loss.data[0], l1_loss2.data[0], l1_loss3.data[0]
                              ))
                    l2_file.write('%s\n' % (str(l2_loss.data[0])))
                    l2_file.flush()
                    l1_file.write('%s\n' % (str(l1_loss.data[0])))
                    l1_file.flush()
                    l3_file.write('%s\n' % (str(l2_loss_depth.data[0])))
                    l3_file.flush()
