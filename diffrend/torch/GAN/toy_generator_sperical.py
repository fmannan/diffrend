...
def sph2cart_unit(u):
    """
    :param u: N x 2 in [radius, azimuth, inclination]
    :return:
    """
    """
...
for epoch in range(opt.niter):
    ...
    for i in range(CRITIC_ITERS):
        ...
        for idx in range(batch_size):
            ...
            large_scene['objects']['disk']['normal'] = sph2cart_unit(np.pi* F.sigmoid(fake[idx][:, 1:]))
            #large_scene['camera']['eye'] = tch_var_f(cam_pos[idx])
            #print(large_scene['objects']['disk']['normal'].size())
            large_scene['camera']['eye'] = tch_var_f(cam_pos)
        ...
        #np.save('normals_working',np.array(normal_array))
