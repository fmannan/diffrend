...
from diffrend.torch.GAN.two_networks_old import create_networks
...
class GAN(object):
    ...
    def render_batch(self, batch, batch_cond=None):
        ...
        print("saved batch")
        out_file2 = os.path.join("/data/lisa/data/sai","scene_output_twogans_color"+".npy")
        np.save(out_file2,scenes)
        out_file3 = os.path.join("/data/lisa/data/sai","res_world_towgans_color"+".npy")
        ...
        with open(file_name, 'wt') as l2_file:
            for iteration in range(self.opt.n_iter):
                ...
                for j in range(self.opt.critic_iters):
                    ...
                    real_output_depth = self.netD2(self.inputv_depth)
                    ...
                    torchvision.utils.save_image(self.inputv.data, os.path.join(self.opt.out_dir,  'input_n%d.png' % (iteration)), nrow=2, normalize=True, scale_each=True)
                    torchvision.utils.save_image(fake_rendered.data, os.path.join(self.opt.out_dir,  'output_n%d.png' % (iteration)), nrow=2, normalize=True, scale_each=True)
                    torchvision.utils.save_image(self.inputv_depth.data, os.path.join(self.opt.out_dir,  'input_depthn%d.png' % (iteration)), nrow=2, normalize=True, scale_each=True)
                    torchvision.utils.save_image(fd.data, os.path.join(self.opt.out_dir,  'output_depthn%d.png' % (iteration)), nrow=2, normalize=True, scale_each=True)
                    break
                    fake_D=torch.cat([fake_rendered.detach(),fd.detach()],1)
                    # Do not bp through gen
                    outD_fake = self.netD(fake_rendered.detach())
                    outD_fake_depth = self.netD2(fd.detach())
                    if self.opt.criterion == 'GAN':
                    ...
                    if self.opt.gp != 'None':
                        ...
                        gradient_penalty_depth = calc_gradient_penalty(
                            self.netD2, self.inputv_depth.data, fd.data,
                            self.opt.gp_lambda)
                ...
                if self.opt.criterion == 'GAN':
                    ...
                    errG = 0.9*outG_fake.mean() + 0.1*outG_fake_depth.mean()
                ...
                if iteration % 2000 == 0:
                    ...
                    np.savetxt('pos_%d.png' % (iteration),r[0]['pos'])
                    np.savetxt('normal_%d.png' % (iteration),r[0]['normal'])
