...
def create_networks(opt, verbose=True, **params):
    ...
    netE2 = LatentEncoder2( nz, 3, nef)
    if opt.netE2 != '':
        netE2.load_state_dict(torch.load(opt.netE2))
    else:
    ...
    if verbose:
        ...
        print(netD2)
        print(netE2)
    return netG, netG2, netD, netD2, netE, netE2
...
class DCGAN_G2(nn.Module):
    ...
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0,
        ...
        main_2.add_module('final_{0}-{1}_convt'.format(cngf, nc),
                        nn.ConvTranspose2d(cngf, 3, 4, 2, 1, bias=True))
        if use_tanh:
...
class _netD(nn.Module):
    def __init__(self, ngpu, nc, ndf, isize, nz, use_sigmoid=0):
        ...
        self.main2 = nn.Sequential(nn.Conv2d((ndf * 12)+(2*nz), 1, 1, 1, 0, bias=True)
...
class LatentEncoder2(nn.Module):
    def __init__(self, nlatent, input_nc, nef):
        super(LatentEncoder2, self).__init__()
