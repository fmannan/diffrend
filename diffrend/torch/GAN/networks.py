...
def create_networks(opt, verbose=True):
    ...
    if opt.disc_type == 'cnn':
        if render_img_size==128:
            if opt.RGBD:
                netD = _netD(ngpu, 4, ndf, render_img_size, use_sigmoid=use_sigmoid)
            else:
                netD = _netD(ngpu, 3, ndf, render_img_size, use_sigmoid=use_sigmoid)
        else:
            if opt.RGBD:
                netD = _netD_64(ngpu, 4, ndf, render_img_size, use_sigmoid=use_sigmoid)
            else:
    ...
    if opt.joint_training:
        if opt.disc_type == 'cnn':
            if render_img_size==128:
                netD2 = _netD2(ngpu, 3, ndf, render_img_size, use_sigmoid=use_sigmoid)
            else:
                netD2 = _netD2_64(ngpu, 3, ndf, render_img_size, use_sigmoid=use_sigmoid)
        elif opt.disc_type == 'dcgan':
            netD2 = DCGAN_D2(render_img_size, nz, render_img_nc, ndf, ngpu,
                           n_extra_layers=disc_nextra_layers,
                           use_sigmoid=use_sigmoid, norm=disc_norm)
        else:
            raise ValueError("Unknown discriminator")
