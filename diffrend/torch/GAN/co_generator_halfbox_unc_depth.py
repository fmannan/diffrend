...
from diffrend.torch.GAN.cnetworks_depth import create_networks
...
class GAN(object):
    ...
    def render_batch(self, batch, batch_cond):
        ...
        for idx in range(batch_size):
            ...
            res_world2 = cam_to_world(pos=self.scene['objects']['disk']['pos'], normal=self.scene['objects']['disk']['normal'], camera=self.scene['camera'])
            # Get rendered output
            if self.opt.render_img_nc == 1:
                depth = res['depth']
                # Normalize depth image
            ...
            rendered_res_world2.append(res_world2)
        rendered_data = torch.stack(rendered_data)
        return rendered_data, rendered_res_world, rendered_res_world2
    ...
    def train(self, ):
        ...
        for iteration in range(self.opt.n_iter):
            ...
            for j in range(self.opt.critic_iters):
                ...
                fake_rendered,r,r2 = self.render_batch(fake,self.inputv_cond)
            ...
            if iteration % 5 == 0:
                fake_rendered_cond,r,r2 = self.render_batch(fake, self.inputv_cond)
                l2_loss=mse_criterion(fake_rendered_cond, self.inputv)
            ...
            if iteration % 20 == 0:
                ...
                plot(r,iteration,self.opt.out_dir)
                plot2(r2,iteration,self.opt.out_dir)
