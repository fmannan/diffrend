...
class Utils(object):
    ...
    def __init__(self, opt):
        ...
        print('CUDA support {}, CUDA actually enabled: {}'.format(
            torch.cuda.is_available(),
            self.CUDA))

        self.tch_var_f = lambda x: tch_var(x, self.FloatTensor, False)
        self.tch_var_l = lambda x: tch_var(x, self.LongTensor, False)

    # only required for meta computation, no tensor req
    def generate_rays(self, camera):
        ...
        x = self.tch_var_f(x.ravel())
        y = self.tch_var_f(y.ravel())

        eye = camera['eye'][:3]
        at = camera['at'][:3]
        up = camera['up'][:3]

        ray_dir = torch.stack((x, y, self.tch_var_f(-np.ones(n_pixels) * focal_length)), dim=0)
    ...
    def lookat(self, eye, at, up):
        ...
        return torch.cat((rot_translate, self.tch_var_f([0, 0, 0, 1])[np.newaxis, :]), dim=0)

    def lookat_inv(self, eye, at, up):
