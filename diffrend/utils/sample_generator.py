...
def uniform_sample_sphere_patch(radius, num_samples, axis, theta_range, phi_range):
...
def uniform_sample_full_sphere(radius, num_samples):
    ...
    return uniform_sample_sphere_patch(radius, num_samples, axis=None, theta_range=[0, np.pi],
                                       phi_range=[0, 2 * np.pi])


def uniform_sample_sphere(radius, num_samples, axis=None, theta_range=None, phi_range=None):
    dispatch_table = [uniform_sample_full_sphere,
                      lambda x, y: uniform_sample_sphere_patch(x, y, axis, theta_range, phi_range)]
    ver = 0 if axis is None and theta_range is None and phi_range is None else 1
